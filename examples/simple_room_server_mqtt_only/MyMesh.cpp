...
void MyMesh::logRx(mesh::Packet *pkt, int len, float score) {
  ...
  if (_prefs.bridge_pkt_src == 1) {  // 1 = bridge on RX
...
#if defined(WITH_MQTT_BRIDGE)
  if (_prefs.bridge_pkt_src == 0) {  // 0 = bridge on TX (default)
    _bridge.sendPacket(pkt);
  }
#endif
  if (_logging) {
...
MyMesh::MyMesh(mesh::MainBoard &board, mesh::Radio &radio, mesh::MillisecondClock &ms, mesh::RNG &rng,
    ...
    : mesh::Mesh(radio, ms, rng, rtc, *(_packetMgr = new StaticPoolPacketManager(32)), tables),
      region_map(key_store), temp_map(key_store),
      _cli(board, rtc, sensors, region_map, acl, &_prefs, this),
      telemetry(MAX_PACKET_PAYLOAD - 4)
#if defined(WITH_MQTT_BRIDGE)
    , _bridge(&_prefs, _packetMgr, &rtc)
#endif
{
  last_millis = 0;
  uptime_millis = 0;
...
#if defined(WITH_MQTT_BRIDGE) && defined(RADIO_NONE)
