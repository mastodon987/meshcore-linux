 ...
 *
 * This is the Linux/Ethernet variant: it talks to the broker over the
 * host's existing wired (or otherwise pre-configured) network connection
 * using a minimal built-in MQTT client (LinuxMQTTClient, raw POSIX TCP
 * sockets). There is no WiFi association step — the OS network stack is
 * assumed to already be up when the repeater starts.
 *
 ...
 *   WITH_MQTT_BRIDGE_PORT=1883        (default 1883)
 ...
 *
 * Note: TLS is not supported by the built-in client. If your broker
 * requires TLS, terminate it locally (e.g. stunnel/socat to 127.0.0.1:1883)
 * and point WITH_MQTT_BRIDGE_SERVER at that local endpoint.
 *
 * All of the above can also be set/overridden at runtime via the CLI:
 *   set bridge.mqtt.server broker.example.com
...
public:
  ...
  /** If bridge.autostart is enabled, connect to the broker immediately.
  ...
  /** Start (or restart) the MQTT bridge. */
  void startMQTT();
  ...
  /** Fills buf with a one-line MQTT status string for the CLI. */
...
private:
  static MQTTBridge* _instance; // for the static MQTT message callback
